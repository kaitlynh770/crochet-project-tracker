...
function Login({ setUser }) {
  ...
  const handleLogin = async (e) => {
    //async function to handle login for users
    e.preventDefault();
    try {
      ...
      <div className={styles.overlay}>
        <div className={styles.login_or_create}>
          ...
          <div className={styles['field-container']}>
            <div className={styles['input-wrapper']} style={{ position: 'relative' }}>
              ...
              <input
                ...
                placeholder={seeLogin ? 'Enter your password' : 'Minimum 6 letters'}
              />
              <img
                src={seePassword ? visible : notVisible}
          ...
          <div className={styles.account_actions}>
            <button className={styles.authentication} type="submit">
              {seeLogin ? 'Login' : 'Create Account'}
            </button>
            <button className={styles.redirect} type="button" onClick={handleToggle}>
              {seeLogin ? 'Dont have an account?' : 'Have an account? Login'}
            </button>
          </div>
        </div>
      </div>
